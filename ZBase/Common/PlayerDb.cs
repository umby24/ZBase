...
namespace ZBase.Common {
	public class PlayerDb {
		...
		public void CreatePlayer(PlayerModel newPlayer) {
			using var context = new PlayerDbContext();
		...
		public PlayerModel GetPlayerModel(string name) {
			using var context = new PlayerDbContext();
			var allPlayers = context.Players.ToList();
			return allPlayers.First(a => String.Equals(a.Name, name, StringComparison.CurrentCultureIgnoreCase));
		}

		public void UpdatePlayer(PlayerModel existingPlayer) {
			using var context = new PlayerDbContext();
			context.Players.Update(existingPlayer);
			context.SaveChanges();
		}

		public void IpBan(IpBanModel ip) {
			using var context = new PlayerDbContext();
			context.IpBans.Add(ip);
			context.SaveChanges();
		}

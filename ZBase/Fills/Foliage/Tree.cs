...
namespace ZBase.Fills.Foliage
{
 public delegate void TreeOutput(ushort x, ushort y, ushort z, BlockID block);
    ...
    { 
        public BlockID LeafBlock = BlockManager.GetBlock("Leaves").Id;
        public BlockID TrunkBlock  = BlockManager.GetBlock("Log").Id;
        ...
        public static bool TreeCheck(HcMap lvl, short x, short y, short z, 
                                     short size, BlockID trunk) {
            for (int dy = -size; dy <= size; ++dy)
                for (int dz = -size; dz <= size; ++dz)
                    for (int dx = -size; dx <= size; ++dx)
            {
                BlockID block = lvl.GetBlockId((short)(x + dx), (short)(y + dy), (short)(z + dz));

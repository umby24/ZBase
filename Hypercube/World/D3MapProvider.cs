...
namespace ZBase.World {
    public class D3MapProvider : IMapProvider {
        ...
        public byte GetBlock(short x, short y, short z) {
            //int index = (z * _underMap.MapSize.Y + y) * _underMap.MapSize.X + x;
            return (byte)_underMap.GetBlock(x, y, z); //MapData[(index * 4)];
        }
...
/*            int index = (x + y * _underMap.MapSize.X + z * _underMap.MapSize.X * _underMap.MapSize.Y) * 4;
            _underMap.MapData[index] = type;*/
        }

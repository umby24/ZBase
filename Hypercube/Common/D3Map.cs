...
/*        public BlockHistory GetHistory(int x, int y, int z) {
            if (!BlockInBounds(x, y, z)) 
                return new BlockHistory();
            
            int index = GetBlockIndex(x, y, z);
            
            var newHistory = new BlockHistory {
                CurrentBlock = MapData[(index * 4)],
                ChangingUser = BitConverter.ToInt16(MapData, (index * 4) + 2)
            };
        ...
        internal int GetBlockIndex(int x, int y, int z) {
            return (y * MapSize.Z + z) * MapSize.X + x;
        }
